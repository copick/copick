...
@pytest.fixture
def test_payload(cryoet_data_portal_payload) -> Dict[str, Any]:
    payload = cryoet_data_portal_payload
    payload["root"] = CopickRootCDP.from_file(payload["cfg_file"])

    dsids = payload["root"].dataset_ids

    root = copick.from_czcdp_datasets(
        dsids,
        payload["root"].config.overlay_root,
        payload["root"].config.overlay_fs_args,
    )

    payload["root_autogen"] = root
    return payload
...
def test_root_lazy(test_payload: Dict[str, Any]):
    ...
    assert len(runs) == 10, "Incorrect number of runs"
    assert set(rnames) == {
        "241",
        "242",
        "243",
...
def test_portal_auto_objects(test_payload: Dict[str, Any]):
    proj = test_payload["root_autogen"]

    assert proj.pickable_objects is not None, "Objects should be populated"
    assert len(proj.pickable_objects) == 14, "Incorrect number of pickable objects"
    assert {o.name for o in proj.pickable_objects} == {
        "cytoplasm",
        "cytosolic-ribosome",
...
def test_root_get_object(test_payload: Dict[str, Any]):
    ...
    objs = ["ribosome", "proteasome", "membrane"]
...
def test_root_refresh(test_payload: Dict[str, Any]):
    ...
    assert len(copick_root.runs) == 10, "Incorrect number of runs"
    assert set(rnames) == {
        "241",
        "242",
        "243",
...
def test_root_new_run(test_payload: Dict[str, Any]):
    # Setup
    copick_root = test_payload["root"]
    test_payload["testfs_overlay"]
    test_payload["testpath_overlay"]
    ...
    with pytest.raises(ValueError):
        copick_root.new_run("241")

    # TODO: Allow adding runs to portal project
    ...
    # copick_root.refresh()
    # assert len(copick_root.runs) == 12, "Incorrect number of runs"
    #
    # # Check filesystem for existing runs
    # run_path = str(overlay_loc / "ExperimentRuns") + "/"
    # for run in ["241", "242", "243", "244", "245", "246", "247", "248", "249", "250", "TS_004", "TS_005"]:
...
# TODO: copy test project objects
...
def test_run_meta(test_payload: Dict[str, Any]):
    ...
    copick_run = copick_root.get_run("241")

    # Name
    assert copick_run.name == "241", "Incorrect name"
...
def test_run_lazy(test_payload: Dict[str, Any]):
    # Setup
    copick_root = test_payload["root_autogen"]
    ...
    assert len(vs) == 1, "Incorrect number of voxel spacings"
    assert set(vs) == {13.480}, "Incorrect voxel spacings"
    ...
    assert len(meshes) == 0, "Incorrect number of meshes"
    ...
    assert len(segmentations) == 8, "Incorrect segmentations"
...
def test_run_get_voxel_spacing(test_payload: Dict[str, Any]):
    ...
    # Get voxel spacing
    vs = copick_run.get_voxel_spacing(13.480)
    assert vs is not None, "Voxel spacing not found"
    assert vs.voxel_size == 13.480, "Incorrect voxel size"
    assert (
...
def test_run_entity_types(test_payload: Dict[str, Any]):
    ...
    assert len(tool_picks) == 8, "Incorrect number of tool picks"

    user_picks = copick_run.user_picks()
    assert len(user_picks) == 0, "Incorrect number of user picks"

    # Check mesh types
    tool_meshes = copick_run.tool_meshes()
    assert len(tool_meshes) == 0, "Incorrect number of tool meshes"
    ...
    assert len(tool_segmentations) == 8, "Incorrect number of tool segmentations"

    user_segmentations = copick_run.user_segmentations()
    assert len(user_segmentations) == 0, "Incorrect number of user segmentations"
...
def test_run_get_picks(test_payload: Dict[str, Any]):
    ...
    assert len(picks) == 0, "Incorrect number of picks"
